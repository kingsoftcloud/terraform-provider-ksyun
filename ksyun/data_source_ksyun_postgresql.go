/*
Query PostgreSQL instance information
...
```hcl
	...
	data "ksyun_postgresql" "search-postgresql"{
	  ...
	  db_instance_type = "HRDS_PG,RR_PG"
	  keyword = ""
	  order = ""
	  project_id = ""
	  marker = ""
...
func dataSourceKsyunPostgresql() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceKsyunPostgresqlRead,
		...
		Schema: map[string]*schema.Schema{
			...
			"db_instance_type": {
				...
				Required:    false,
				Optional:    true,
				Description: "HRDS_PG (highly available), RR_PG (read-only).",
			},
			"db_instance_status": {
				Type:        schema.TypeString,
				Required:    false,
			...
			"postgresql": {
				...
				//Optional:    true,
				Computed:    true,
				Description: "An information list of PostgreSQL.",
...
func dataSourceKsyunPostgresqlRead(d *schema.ResourceData, meta interface{}) error {
	...
	if !dataOk {
		return fmt.Errorf("error on reading Instance(postgresql) body %q, %+v", d.Id(), (*resp)["Error"])
	}
	...
	for num, instance := range instances {
		...
		for k, v := range instanceInfo {
			if k == "DBInstanceClass" {
				...
				postgresqlMap["db_instance_class"] = wtf
			} else {
				dk := Camel2Hungarian(k)
				if _, ok := dataSourceKsyunPostgresql().Schema["postgresql"].Elem.(*schema.Resource).Schema[dk]; ok {
					postgresqlMap[dk] = v
				}
			}
		...
		postgresqlIds[num] = postgresqlMap["db_instance_identifier"].(string)
	...
	_ = dataDbSave(d, "postgresql", postgresqlIds, postgresqlMapList)

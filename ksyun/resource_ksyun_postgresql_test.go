...
func TestAccKsyunPostgresql_basic(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		IDRefreshName: "ksyun_postgresql.rds_terraform_3",
		Providers:     testAccProviders,
		CheckDestroy:  testAccCheckPostgresqlDestroy,

		Steps: []resource.TestStep{
			{
				Config: testAccPostgresqlConfig,

				Check: resource.ComposeTestCheckFunc(
					testCheckPostgresqlExists("ksyun_postgresql.rds_terraform_3", &val),
				),
			},
			{
				Config: testAccPostgresqlEipConfig,
				Check: resource.ComposeTestCheckFunc(
					testCheckPostgresqlExists("ksyun_postgresql.rds_terraform_3", &val),
				),
			},
...
func testCheckPostgresqlExists(n string, val *map[string]interface{}) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		resp, err := client.postgresqlconn.DescribeDBInstances(&req)
		if err != nil {
			return err
		}
		if resp != nil {
			...
			if !dataOk {
				return fmt.Errorf("error on reading Instance(postgresql)  %+v", (*resp)["Error"])
			}
...
func testAccCheckPostgresqlDestroy(s *terraform.State) error {
	for _, res := range s.RootModule().Resources {
		...
		_, err := client.postgresqlconn.DescribeDBInstances(&req)
		if err != nil {
...
resource "ksyun_postgresql" "rds_terraform_3"{
  ...
  db_parameter_template_id = ksyun_postgresql_parameter_group.dpg_with_parameters.id
  parameters {
    name = "auto_increment_increment"
    value = "8"
  }
...
resource "ksyun_tag" "test_tag" {
    ...
    resource_id = "${ksyun_postgresql.rds_terraform_3.id}"
}

`
const testAccPostgresqlEipConfig = `
variable "available_zone" {
  default = "cn-beijing-6a"
}
resource "ksyun_vpc" "default" {

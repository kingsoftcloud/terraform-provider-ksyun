...
- [Terraform](https://www.terraform.io/downloads.html) v1.0.x
...
```bash
TF_LOG=debug TF_ACC=true go test -test.run TestAccKsyunEip_basic -v
```
...
##### 6、网络和主机的接口属性较多，开发采用了函数封装，初次开发，可先参考金山云Terraform Provider开发指南，更有易读性，了解terraform的开发过程后可直接调用封装好的函数，减少代码冗余。
